...
uint32_t Bsa::getFileCount() const noexcept {
  try {
    ...
    case FO3:
    case SSE:
      return get<HeaderTES4>(m_header).fileCount;
    case FO4:
    case FO4dds:
      return get<HeaderFO4>(m_header).fileCount;
    case SF:
      return get<HeaderSF>(m_header).fo4Header.fileCount;
    case SFdds:
      return get<HeaderSFdds>(m_header).fo4Header.fileCount;
    default:
      return 0;
    }
...
Bsa::Bsa(const filesystem::path& archivePath, ArchiveType type, std::vector<std::filesystem::path>& fileList,
...
FileRecord_t Bsa::findFileRecord(const std::filesystem::path& fileName) noexcept {
  try {
    ...
    case FO3:
    case SSE:
      return findFileRecordTES4(fileName);
    case FO4:
    case FO4dds:
    case SF:
    case SFdds:
...
void Bsa::decompressData(const Buffer& compressed, uint8_t* uncompressed, uint32_t uncompressedSize) const
  ...
  case zlib: {
    ...
    uncompress(uncompressed, reinterpret_cast<uLongf*>(&dstSize), compressed.data(),
               gsl_lite::narrow<uLong>(compressed.size()));

    if (dstSize != uncompressedSize) {
      throw runtime_error(
          format("Decompression error: dstSize has unexpected size, expected {}, got {}", uncompressedSize, dstSize));
    }
  ...
  case lz4Frame: {
    ...
    const unique_ptr<LZ4F_dctx, decltype(&LZ4F_freeDecompressionContext)> dctx{dctxPtr, LZ4F_freeDecompressionContext};
  ...
  case lz4Block:
    LZ4_decompress_safe(reinterpret_cast<const char*>(compressed.data()), reinterpret_cast<char*>(uncompressed),
                        gsl_lite::narrow<int>(compressed.size()), gsl_lite::narrow<int>(uncompressedSize));
...
void Bsa::save() noexcept(false) {
  ...
  default:
    throw runtime_error("Invalid archive type for saving");
  }
}
...
Buffer Bsa::compressData(const uint8_t* data, size_t length) const noexcept(false) {
  ...
  case lz4Block: {
    ...
    // compress
    const int result =
        LZ4_compress_default(reinterpret_cast<const char*>(data), reinterpret_cast<char*>(dst.data()), srcSize, bound);
...
Bsa::getFileList(const std::optional<std::filesystem::path>& directoryName) const noexcept {
...
void Bsa::extract(const filesystem::path& archivePath, const filesystem::path& outputDirectory,
